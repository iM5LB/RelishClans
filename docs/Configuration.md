...
Under `flags.definitions` each flag has gameplay settings only:
...
Default unlocked at clan create: `flags.default-unlocked`.
...
| **Detail flags** | `mob_spawn_control`, `pvp_mode`, `no_pvp`, `heal_zone`, `alarm_bell`, `visitor_greeting`, `explosion_shield`, `build_lock`, `container_lock`, `use_lock`, `entity_interact`, `villager_lock`, `spawner_lock` — same clicks, plus shift-click opens the settings screen. |
...
debug-mode: false
config-version: 2   # do not edit — used by config updater
```
