// GitBook-style documentation for RelishClans (Font Awesome icons, no emoji)
...
const PAGES = [
  ...
  { file: 'Claims.md', title: 'Claims & Core', section: 'Features', icon: 'fa-solid fa-map' },
  { file: 'Wars.md', title: 'Wars & Diplomacy', section: 'Features', icon: 'fa-solid fa-shield-halved' },
  { file: 'Flags.md', title: 'Flags & Upgrades', section: 'Features', icon: 'fa-solid fa-flag' },
...
function buildSidebar(filter = '') {
  ...
  PAGES.forEach(p => {
    ...
    html += `<a href="#/${encodeURIComponent(p.file)}" data-file="${p.file}"><span class="nav-icon">${iconHtml(p.icon)}</span><span class="nav-text"><span class="nav-label">${p.title}</span>${snippet ? `<span class="nav-snippet">${escapeHtml(snippet)}</span>` : ''}</span></a>`;
  });
